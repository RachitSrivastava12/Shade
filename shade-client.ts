...
export class ShadeClient {
  ...
  async fetchBookER(): Promise<BookJSON> {
    return normalizeBook(await (this.erProgram.account as any).orderBook.fetch(this.book));
  }
  /** Read the book from the base layer (last committed state). */
  async fetchBookBase(): Promise<BookJSON> {
    return normalizeBook(await (this.program.account as any).orderBook.fetch(this.book));
  }
}
